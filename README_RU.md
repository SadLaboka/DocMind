...
2. **Сохранение метаданных** (PostgreSQL)
   ...
   - Статус: `created` → `scanning`/`extracting`
...
**Почему S3-совместимое хранилище?**
- **Stateless-архитектура**: Воркеры могут масштабироваться горизонтально без необходимости синхронизации локальных файлов
- **Надёжность**: Файлы не теряются при перезапуске контейнеров
- **Безопасность**: Presigned URL с ограниченным временем жизни для безопасного скачивания
- **Гибкость**: Поддержка различных S3-совместимых хранилищ (MinIO для локальной разработки, AWS S3 / Selectel Cloud для продакшена)
...
| Переменная | Описание | По умолчанию |
|------------|----------|--------------|
| `ANTIVIRUS_ENABLED` | Включить антивирусную проверку | `true` |
| `ANTIVIRUS_HOST` | Хост ClamAV | `clamav` |
| `ANTIVIRUS_PORT` | Порт ClamAV | `3310` |
...
| `GEMINI_MODEL` | Название модели | `gemini-3.1-flash-lite` |
| `GEMINI_TIMEOUT` | Таймаут запроса (секунды) | `60` |
