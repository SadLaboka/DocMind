...
def process_upload(
    uploaded_file: UploadFile, user_id: int, description: str | None
    ...
    file_size = _validate_size(uploaded_file)

    file_extension = Path(uploaded_file.filename).suffix.lower()
    filename = Path(uploaded_file.filename).stem
    temp_filename = _get_temp_filename(file_extension)
    sanitized_filename = _sanitize_filename(filename)

    mime_type = _detect_mime(uploaded_file)
    ...
    _save_to_temp(file=uploaded_file, temp_name=temp_filename)
