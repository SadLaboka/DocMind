...
async def test_upload_document_extracting_only_path_different_providers(
    ...
    with patch("src.services.file_processor.asyncio.to_thread") as mock_to_thread:
        ...
        response = await client.post(
            "/documents/",
            files=files,
            data={"description": "Duplicate upload", "provider": "gemini"},
            headers={"Authorization": f"Bearer {tokens['access_token']}"},
        )
...
async def test_upload_document_analyzing_only_path_cross_user(
    client: AsyncClient, create_token_pair, create_document, test_password, test_db_session, mock_mongo_content
):
    _, hashed_pw = test_password
    tokens = await create_token_pair(login="uploader1", email="up@test.com", password_hash=hashed_pw)
    tokens2 = await create_token_pair(login="uploader2", email="up2@test.com", password_hash=hashed_pw)
