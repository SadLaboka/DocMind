...
async def test_register_success(client: AsyncClient):
    response = await client.post("users/register",
                                 json={"login": "test", "email": "test@test.com", "password": "testpw1234"})
...
async def test_register_validation_login_too_short(client: AsyncClient):
    response = await client.post(
        "/users/register",
        json={"login": "abc", "email": "test@test.com", "password": "securepass123"}
    )
...
async def test_register_validation_login_too_long(client: AsyncClient):
    response = await client.post(
        "/users/register",
        json={"login": "a" * 26, "email": "test@test.com", "password": "securepass123"}
    )
    assert response.status_code == 422
...
async def test_register_validation_invalid_email(client: AsyncClient):
    response = await client.post(
        "/users/register",
        json={"login": "validuser", "email": "not-an-email", "password": "securepass123"}
    )
    assert response.status_code == 422
...
async def test_register_validation_password_too_short(client: AsyncClient):
    response = await client.post(
        "/users/register",
        json={"login": "validuser", "email": "test@test.com", "password": "123"}
    )
    assert response.status_code == 422
...
async def test_register_validation_password_too_long(client: AsyncClient):
    response = await client.post(
        "/users/register",
        json={"login": "validuser", "email": "test@test.com", "password": "a" * 65}
    )
    assert response.status_code == 422
...
async def test_register_duplicate_login(client: AsyncClient, create_user, test_db_session, test_password):
    ...
    response = await client.post(
        "/users/register",
        json={"login": "duplicate_login", "email": "second@test.com", "password": "securepass123"}
    )
